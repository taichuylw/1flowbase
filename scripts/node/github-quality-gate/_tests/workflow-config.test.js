...
test("verify workflow runs quality gate scopes in parallel before one aggregate report", () => {
  ...
  assert.match(
    workflow,
    /repo-tooling-gate:[\s\S]*?git fetch --no-tags --prune origin main:refs\/remotes\/origin\/main/u,
  );
...
test("quality gate workflow keeps non-ci dispatch scopes on a single targeted job", () => {
  ...
  assert.match(
    workflow,
    /single-scope-gate:[\s\S]*?git fetch --no-tags --prune origin main:refs\/remotes\/origin\/main/u,
  );
