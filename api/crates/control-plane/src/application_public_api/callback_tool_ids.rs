...
pub const OPENAI_CALLBACK_TOOL_CALL_PREFIX: &str = "calltask_";
pub const ANTHROPIC_CALLBACK_TOOL_USE_PREFIX: &str = "toolu_task_";

pub fn encode_openai_callback_tool_call_id(
...
pub fn decode_openai_callback_tool_call_id(value: &str) -> Option<(Uuid, String)> {
    let rest = value.strip_prefix(OPENAI_CALLBACK_TOOL_CALL_PREFIX)?;
    decode_callback_bound_id(rest)
}

pub fn encode_anthropic_callback_tool_use_id(
    callback_task_id: Uuid,
...
pub fn decode_anthropic_callback_tool_use_id(value: &str) -> Option<(Uuid, String)> {
