...
describe('FrontStagePage', () => {
  ...
  test('opens page tree operation menu on hover', async () => {
    ...
    const promptSpy = vi
      .spyOn(window, 'prompt')
      .mockReturnValue('展示在页面树');

    try {
      fireEvent.click(await screen.findByText('编辑提示信息'));
      await waitFor(() => {
        expect(onUpdatePageNodeMetadata).toHaveBeenCalledWith('page-1', {
          tooltip: '展示在页面树'
        });
  ...
  test('falls back to first available page when selected page is deleted by parent group', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByRole('heading', { name: '页面 新建 2' })
      ).toBeInTheDocument();
    });
  ...
  test('falls back to workspace-level route when selected nested group is deleted and no pages remain', () => {
    ...
    expect(
      screen.getByRole('heading', { name: '未选择 pageId（将使用默认首页）' })
    ).toBeInTheDocument();
