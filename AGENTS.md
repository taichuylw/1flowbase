...
# 本项目 skills
1.`problem-framing`：非平凡功能、架构、交互、重构、规则或跨前后端需求动工前使用，先收敛目标、范围、成功标准、假设、风险、方案和用户拍板点。
...
# 开发流程控制
1.非平凡需求先使用 `problem-framing`，完成事实/假设分离、范围收敛、终止条件和用户拍板，再进入实现类 skill。
2.`problem-framing` 阶段不得修改产品代码；命中三方案场景时，按“现状、方向、风险收益、建议”输出保守/平衡/激进方案并等待用户拍板。
3.纯局部 bugfix、文案、样式 token、机械重命名，或用户明确要求直接开始时，可跳过 `problem-framing`。
