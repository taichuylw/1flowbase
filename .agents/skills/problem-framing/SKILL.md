...
需求类请求未完成对齐和用户确认前，不进入代码实现。方案确认只授权进入 issue 草案 / 审核，不等于授权实现；除非用户明确说跳过 issue 或直接实现，否则没有已确认 L3 issue 不进入实现。影响数据、contract、架构或用户内容的请求，未完成事实整理、范围收敛和用户拍板前，不进入迁移设计或大规模重构计划。需要开发计划时，不跳过 L0 事实、L1 决策、L2 工作流和 L3 执行边界；实现阶段不得用 L3 修改 L1 已定架构边界。
