...
summary: 用户在 `2026-05-27 00` 确认多语言资源采用“就近 owner + 脚本统一检查”的平衡方案：前端 UI 只做 `zh-CN/en-US`，插件/provider 使用 `zh_Hans/en_US`；同 owner 内 key/value 问题由 `i18n-hygiene` 阻塞，跨 owner 重复 key/value 先以 warning 暴露，AI 在 QA 或修复时根据 owner 语义决定是否上提 common。
keywords:
  - i18n
  - i18n-hygiene
  - frontend
